...
const ships = [
  { id: 1, name: "Garuda Nusantara", lat: -5.0406, lng: 119.3250, heading: 45, speed: 12.5, type: "Nelayan Kecil" },
  { id: 2, name: "Samudra Jaya", lat: -5.0450, lng: 119.3300, heading: 270, speed: 8.3, type: "Kapal Penumpang" },
  { id: 3, name: "Bahari Express", lat: -5.0350, lng: 119.3200, heading: 180, speed: 15.7, type: "Kapal Penumpang" },
  { id: 4, name: "Nusa Bahari", lat: -5.0380, lng: 119.3150, heading: 315, speed: 10.2, type: "Nelayan Besar" },
];
  ...
  useEffect(() => {
    ...
    async function initializeMap() {
      if (typeof window !== "undefined" && mapRef.current && !leafletMap.current) {
        ...
        // Create map instance
        const map = L.map(mapRef.current).setView([-5.0406, 119.3250], 15);
        ...
        ships.forEach((ship) => {
          ...
          const headingLine = L.polyline(
            [
              [ship.lat, ship.lng],
              [
                ship.lat + 0.01 * Math.sin((ship.heading * Math.PI) / 180),
                ship.lng + 0.01 * Math.cos((ship.heading * Math.PI) / 180),
              ],
            ],
            {
              color: "#3b82f6",
    ...
    <div className="relative w-full h-full bg-gray-100">
      ...
      <div className="absolute top-4 right-4 z-20 flex justify-between items-center pointer-events-none">
        <div className="bg-background/80 backdrop-blur-sm p-2 rounded-md flex items-center space-x-2 pointer-events-auto">
          ...
          <span className="text-xs text-foreground">{time.toLocaleTimeString()} WIB</span>
        </div>
      </div>
